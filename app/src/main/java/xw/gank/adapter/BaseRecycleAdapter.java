 ...
 * 添加头部以及尾部的RecycleView
 * 使用步骤
 * 创建一个子类继承BaseRecycleAdapter<T>
 * 重写bindData方法
 * 此方法中绑定View hodle.getView(id)
 */
...
public abstract class BaseRecycleAdapter<T> extends RecyclerView.Adapter<BaseRecycleAdapter.BaseViewHolder> {
    ...
    BaseRecycleAdapter(List<T> datas) {
        this.datas = datas;
    }
    ...
    // item 的三种类型
    private static final int ITEM_TYPE_NORMAL = 0X1111; // 正常的item类型
    private static final int ITEM_TYPE_HEADER = 0X1112; // header
    private static final int ITEM_TYPE_FOOTER = 0X1113; // footer
